...
#' ####################################
 # computation time
tic(); FIM_compiled <- evaluate.fim(babel.db); toc()

#' design evaluation
crit <- det(FIM_compiled)^(1/length(get_unfixed_params(babel.db)[["all"]]))
crit
rse <- get_rse(FIM_compiled,babel.db) # this is for the log of the fixed effect parameters
