...
To use `Monolix` with nlmixr2, you do not need to change your data or
your `nlmixr2` dataset. `babelmixr2` will do the heavy lifting here.
...
often useful to use the full command path and set it in the options, ie
...
Once monolix is run, you can run the `nlmixr2` model using `Monolix` as
if it is new estimation method:
...
#> ℹ monolix parameter history needs exported charts, please export charts
```
...
don’t have that information then the important parameter history plots
will not be imported and you cannot see those plots.

Just like with the NONMEM translation, the
[`monolixControl()`](https://nlmixr2.github.io/babelmixr2/reference/monolixControl.md)
has `modelName` which helps control the output directory of `Monolix`
...
Of particular interest is the comparison between Monolix predictions and
nlmixr predictions. In this case, I believe that these also imply the
models are predicting the same thing. Note that the model predictions
are not as close as they were with
[NONMEM](https://nlmixr2.github.io/babelmixr2/articles/running-nonmem.html)
because Monolix does not use the `lsoda` ODE solver. Hence this small
deviation is expected, but still gives a validated Monolix model.
...
To be fair, objective function values must always be used with caution.
How the model performs and predicts the data is far more valuable.
...
![](running-monlix_files/figure-html/vpc-1.png)
...
![](running-monlix_files/figure-html/vpc-2.png)

## Notes about Monolix data translation

The input dataset expected to be compatible with `rxode2` or `nlmixr2`.
This dataset is then converted to Monolix format:

- The combination of `CMT` and Dose type creates a unique `ADM`
  variable.

- The `ADM` definition is saved in the monolix model file

- `babelmixr2` creates a macro describing the compartment, ie
  `compartment(cmt=#, amount=stateName)`

- `babelmixr2` also creates a macro for each type of dosing:

  - Bolus/infusion uses `depot()` and adds modeled lag time (`Tlag`) or
    bioavailability (`p`) if specified
