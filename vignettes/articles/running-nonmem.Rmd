...
The `monolixControl()` has `modelName` which helps control the output
directory of `Monolix` (if not specified `babelmixr2` tries to guess
based on the model name based on the input).
...
Of particular interest is the comparison between Monolix predictions and nlmixr predictions, which shows the models are quite close.

In this case, I believe that these also imply the models are
predicting the same thing. 

Note that the model predictions are not as
